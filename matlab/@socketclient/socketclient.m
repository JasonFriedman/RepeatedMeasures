...
params.type = {'string','number','string','cellarray','matrix','cellarray','matrix_n_1','cellarray','cellarray','matrix','matrix'};
params.description = {'Location of the server (e.g. ''localhost'' or an IP address) ',...
    ...
    'If showing position, the center of rotation (if showPositionRotationAngle is not zero). Should be a P * 2 matrix (p = number of dots)',...
    'If showing position, the angle to rotate about the center of rotation (in radians). Should be a P * 1 matrix (p = number of dots)',...
