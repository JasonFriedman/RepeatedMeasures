...
params.type = {'string','number','string','cellarray','matrix',...
    'cellarray','cellarray','matrix_n_1','cellarray','cellarray','matrix','matrix'};
params.description = {'Location of the server (e.g. ''localhost'' or an IP address) ',...
    ...
    'If showing position, the frame to start the rotation (if there is one). To start before the trial (i.e. while waiting for start), set to -inf',...
...
for k=1:numel(sc.showPositionRotationAngle)
    if ischar(sc.showPositionRotationAngle{k})
