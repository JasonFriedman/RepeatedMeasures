...
params.description = {'Location of the server (e.g. ''localhost'' or an IP address) ',...
    ...
    ['If showing position, how to show the current position (either ''dot'',''ellipse'' or ''rectangle''). Note that if showPosition=1, it will always be shown. If showPosition=2 and imagestimulus is used,' ...
    'it will only be shown when in state 1']',...
    'If showing position, the color of the feedback (each number between 0 and 255). If it is a cell array, then each cell can specify for an individual trial.',...
