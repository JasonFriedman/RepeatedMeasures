...
% Set the measurement rate (note that can receive samples at 3 times the
% measurement rate)
handleError(ts,calllib(ts.libstring, 'SetSystemParameter',...
    enuminfo.SYSTEM_PARAMETER_TYPE.MEASUREMENT_RATE, double(ts.samplerate/3), 8));
...
for sensor = 1:(ceil(numSensors/4)*4)               
