...
params.description = {['An n x 4 or n x 6 matrix. The first column is the image number, the second the start time, ' ...
    ...
    ' (from the positions in targetPosition), the third column the distance allowed, the fourth column whether the pen must be touching (If not using a tablet set to 0, for a tablet 0 = doesn''t matter, 1 = must touch, 2 = must not touch)',...
